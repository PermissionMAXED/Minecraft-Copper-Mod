...
# Inferno biomes the Ember Swarm spawns in. The canonical 7-biome list of the Inferno
# dimension lives in devtools/gen/infernodim2_gen.py (BIOMES/CLIMATE_POINTS). The swarm
# infests every biome: full weights in the six busy biomes, and a second QUIET predicate
# for crystal_hollows where every addSpawn weight is halved (min 1) to preserve the
# hushed crystal-cavern theme (its biome JSON curates vanilla monsters down to endermen).
SPAWN_BIOMES = ["cinder_wastes", "ember_grove", "slag_sea",
                "verdigris_jungle", "molten_delta", "soot_dunes"]
QUIET_SPAWN_BIOMES = ["crystal_hollows"]
...
def feature_java() -> str:
    doc = [
        ...
        "natural spawns across the seven Inferno biomes (full weights everywhere except the",
        "quiet crystal_hollows, where every weight is halved) and drop recipes are registered",
        "here;",
    ...
    out += ["",
            "\t\t// Natural spawns across the Inferno biomes (SPAWN_BIOMES in the generator;",
            "\t\t// biome JSONs are owned by infernodim/infernodim2 and includeByKey matches",
            "\t\t// nothing until they are loaded).",
    ...
    out += ["",
            "\t\t// crystal_hollows is curated as QUIET: same swarm roster, every weight",
            "\t\t// halved (min 1), so the hushed crystal caverns stay sparsely infested.",
            "\t\tPredicate<BiomeSelectionContext> quietBiomes = BiomeSelectors.includeByKey("]
    for i, biome in enumerate(QUIET_SPAWN_BIOMES):
        suffix = ");" if i == len(QUIET_SPAWN_BIOMES) - 1 else ","
        out.append(f"\t\t\t\tRegistryKey.of(RegistryKeys.BIOME, CopperInferno.id(\"{biome}\")){suffix}")
    for mob in MOBS:
        out.append(f"\t\tBiomeModifications.addSpawn(quietBiomes, SpawnGroup.MONSTER, "
                   f"{field(mob.mid)}, {max(1, mob.weight // 2)}, {mob.gmin}, {mob.gmax});")
    out += ["\t}", ""]
