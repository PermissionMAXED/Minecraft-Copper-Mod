...
def gen_feature_java():
    ...
    out.append("")
    out.append("\t\t// Natural spawns in the three Inferno biomes (biome JSONs are owned by the")
    out.append("\t\t// infernodim feature; includeByKey simply matches nothing until they are loaded).")
    out.append("\t\tPredicate<BiomeSelectionContext> infernoBiomes = BiomeSelectors.includeByKey(")
    out.append("\t\t\t\tRegistryKey.of(RegistryKeys.BIOME, CopperInferno.id(\"cinder_wastes\")),")
    out.append("\t\t\t\tRegistryKey.of(RegistryKeys.BIOME, CopperInferno.id(\"ember_grove\")),")
    out.append("\t\t\t\tRegistryKey.of(RegistryKeys.BIOME, CopperInferno.id(\"slag_sea\")));")
    for mob in MOBS:
