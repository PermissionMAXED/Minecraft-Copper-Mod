...
# CHAIN (no cycle): material[0] (ember_brass) is the ENTRY, crafted from obtainable
# ingredients (copper_ingot corners + slag_chunk centre) instead of material[13].
...
def emit_recipes() -> int:
    ...
    for i, mat in enumerate(MATERIALS):
        ...
        # material[0] (ember_brass) is the chain ENTRY: crafted from already-obtainable
        # ingredients (copper_ingot corners + slag_chunk centre, slag_chunk = 2x2
        # ash_pile per infernium_gen.py) so the 14-link chain has an entry point and no
        # crafting cycle. Materials i >= 1 use 4x vanilla flavor ingredient around the
        # PREVIOUS material's base block.
        if i == 0:
            corner, centre = "minecraft:copper_ingot", f"{ci}slag_chunk"
        else:
