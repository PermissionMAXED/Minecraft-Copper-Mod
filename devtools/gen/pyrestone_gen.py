#!/usr/bin/env python3
"""Asset + code generator for the COPPER INFERNO "pyrestone" feature (224 building blocks).

14 volcanic stone materials (pyrestone, emberstone, sootstone, charflint, kindlerock,
cindershale, coalspar, tindergrit, flarebasalt, magmite, fumarole_rock, pyroclast_stone,
vitrified_ash, blazewrought_stone); each ships a 16-block set: the base cube family, the
polished family and the bricks family (base/slab/stairs/wall each), plus tiles, cracked
bricks, chiseled bricks and a pillar.
...
noise is seeded per texture name via genlib.rng_for). Emits by DEFAULT (no flags):
  ...
  - 16x16 block textures (Pillow, deterministic seeded noise, distinct palette per material)
  - loot tables (drop-self; slabs use the vanilla double-drops-2 format)
  - recipes (data/copper_inferno/recipe/pyrestone/*.json, 30 per material = 420)
  - lang fragments: assets/copper_inferno/lang/fragments/pyrestone.json (EN)
    and assets/copper_inferno/lang/fragments_de/pyrestone.json (real German)
  - devtools/tagfrag/pyrestone.json (mineable/pickaxe for all 224, walls for the 42 walls)
  - src/main/java/.../feature/pyrestone/PyrestoneFeature.java + PyrestoneHandbook.java
    (genlib.java_feature_class / genlib.java_handbook_class; literal ids only)
  - devtools/hooks/pyrestone.txt (integration hook file)
...
RECIPES = DATA / "recipe" / "pyrestone"
FEATURE_DIR = ROOT / "src" / "main" / "java" / "net" / "sonic0810" / "copperinferno" / "feature" / "pyrestone"

# ---------------------------------------------------------------------------
# Materials. Each gets a DISTINCT deterministic palette:
...
# CHAIN (no cycle): material[0] (pyrestone) is the ENTRY, crafted from obtainable
# Inferno materials (cinderstone corners + ember_dust centre) instead of material[13].
# Every crafting recipe still contains at least one own id, so it can never collide
# with vanilla or other features' recipes.
# ---------------------------------------------------------------------------
Mat = namedtuple("Mat", "mid en de de_stem de_pol vanilla map_color sounds "
                        "shades mortar accents base_prob brick_prob")

MATERIALS = [
    Mat("pyrestone", "Pyrestone", "Pyrostein", "Pyrostein", "Polierter",
        "minecraft:granite", "TERRACOTTA_ORANGE", "STONE",
        [(0x5A, 0x2A, 0x1C), (0x74, 0x38, 0x24), (0x74, 0x38, 0x24), (0x8F, 0x4A, 0x2E)],
        (0x38, 0x18, 0x10), [(0xE2, 0x58, 0x22), (0xFF, 0x7A, 0x2F)], 0.03, 0.10),
    Mat("emberstone", "Emberstone", "Glutstein", "Glutstein", "Polierter",
        "minecraft:netherrack", "ORANGE", "NETHERRACK",
        [(0x6E, 0x30, 0x14), (0x8C, 0x40, 0x1A), (0x8C, 0x40, 0x1A), (0xAE, 0x55, 0x22)],
        (0x45, 0x1D, 0x0C), [(0xFF, 0x7A, 0x2F), (0xFF, 0xB1, 0x6B)], 0.05, 0.20),
    Mat("sootstone", "Sootstone", "Ru\u00dfstein", "Ru\u00dfstein", "Polierter",
        "minecraft:tuff", "GRAY", "TUFF",
        [(0x2A, 0x2A, 0x2C), (0x39, 0x39, 0x3C), (0x39, 0x39, 0x3C), (0x4A, 0x4A, 0x4E)],
        (0x1A, 0x1A, 0x1C), [(0x5E, 0x5E, 0x64), (0x77, 0x77, 0x7E)], 0.02, 0.0),
    Mat("charflint", "Charflint", "Schwarzflint", "Schwarzflint", "Polierter",
        "minecraft:flint", "BLACK", "DEEPSLATE",
        [(0x17, 0x17, 0x1A), (0x24, 0x24, 0x28), (0x24, 0x24, 0x28), (0x33, 0x33, 0x39)],
        (0x0D, 0x0D, 0x10), [(0x4C, 0x4E, 0x58), (0x6A, 0x6E, 0x7A)], 0.03, 0.06),
    Mat("kindlerock", "Kindlerock", "Z\u00fcndelstein", "Z\u00fcndelstein", "Polierter",
        "minecraft:packed_mud", "BROWN", "STONE",
        [(0x4E, 0x37, 0x22), (0x64, 0x47, 0x2C), (0x64, 0x47, 0x2C), (0x7C, 0x59, 0x38)],
        (0x30, 0x21, 0x13), [(0xE2, 0x58, 0x22), (0xB8, 0x8A, 0x52)], 0.03, 0.08),
    Mat("cindershale", "Cindershale", "Aschenschiefer", "Aschenschiefer", "Polierter",
        "minecraft:deepslate", "LIGHT_BLUE_GRAY", "DEEPSLATE",
        [(0x33, 0x3C, 0x4A), (0x43, 0x4E, 0x60), (0x43, 0x4E, 0x60), (0x56, 0x64, 0x78)],
        (0x20, 0x26, 0x30), [(0x7C, 0x90, 0xAA), (0x9A, 0xB2, 0xCC)], 0.02, 0.05),
    Mat("coalspar", "Coalspar", "Kohlenspat", "Kohlenspat", "Polierter",
        "minecraft:coal_block", "TERRACOTTA_BLACK", "DEEPSLATE",
        [(0x14, 0x12, 0x16), (0x20, 0x1D, 0x23), (0x20, 0x1D, 0x23), (0x2E, 0x2A, 0x32)],
        (0x0A, 0x09, 0x0C), [(0xE8, 0xE2, 0xC8), (0xFF, 0xF6, 0xD8)], 0.04, 0.06),
    Mat("tindergrit", "Tindergrit", "Zundergrus", "Zundergrus", "Polierter",
        "minecraft:sandstone", "PALE_YELLOW", "STONE",
        [(0x8A, 0x74, 0x4E), (0xA4, 0x8C, 0x60), (0xA4, 0x8C, 0x60), (0xBE, 0xA6, 0x74)],
        (0x62, 0x50, 0x34), [(0xD8, 0xC2, 0x8E), (0xE2, 0x58, 0x22)], 0.03, 0.06),
    Mat("flarebasalt", "Flarebasalt", "Flackerbasalt", "Flackerbasalt", "Polierter",
        "minecraft:basalt", "DEEPSLATE_GRAY", "BASALT",
        [(0x2A, 0x28, 0x2C), (0x38, 0x36, 0x3B), (0x38, 0x36, 0x3B), (0x48, 0x45, 0x4B)],
        (0x18, 0x17, 0x1A), [(0xE2, 0x58, 0x22), (0xFF, 0x7A, 0x2F)], 0.04, 0.16),
    Mat("magmite", "Magmite", "Magmit", "Magmit", "Polierter",
        "minecraft:magma_block", "DARK_RED", "NETHER_BRICKS",
        [(0x58, 0x16, 0x0E), (0x71, 0x1F, 0x12), (0x71, 0x1F, 0x12), (0x8E, 0x2B, 0x16)],
        (0x33, 0x0B, 0x07), [(0xFF, 0x64, 0x28), (0xFF, 0x9A, 0x40)], 0.06, 0.25),
    Mat("fumarole_rock", "Fumarole Rock", "Fumarolenstein", "Fumarolenstein", "Polierter",
        "minecraft:andesite", "TERRACOTTA_YELLOW", "CALCITE",
        [(0x6E, 0x68, 0x4A), (0x86, 0x80, 0x5C), (0x86, 0x80, 0x5C), (0x9E, 0x97, 0x6E)],
        (0x49, 0x44, 0x2E), [(0xC8, 0xB8, 0x64), (0xE0, 0xD2, 0x7E)], 0.03, 0.05),
    Mat("pyroclast_stone", "Pyroclast Stone", "Pyroklaststein", "Pyroklaststein", "Polierter",
        "minecraft:cobblestone", "STONE_GRAY", "STONE",
        [(0x4A, 0x40, 0x38), (0x5D, 0x51, 0x47), (0x5D, 0x51, 0x47), (0x71, 0x63, 0x56)],
        (0x2E, 0x27, 0x21), [(0xE2, 0x58, 0x22), (0x8E, 0x7C, 0x6C)], 0.03, 0.07),
    Mat("vitrified_ash", "Vitrified Ash", "Glasasche", "Glasaschen", "Polierte",
        "minecraft:calcite", "LIGHT_GRAY", "CALCITE",
        [(0x8E, 0x8C, 0x88), (0xA6, 0xA4, 0xA0), (0xA6, 0xA4, 0xA0), (0xBE, 0xBC, 0xB8)],
        (0x6A, 0x68, 0x64), [(0xD8, 0xD6, 0xD0), (0xEC, 0xEA, 0xE4)], 0.02, 0.0),
    Mat("blazewrought_stone", "Blazewrought Stone", "Flammschmiedestein", "Flammschmiedestein",
        "Polierter", "minecraft:gilded_blackstone", "GOLD", "NETHER_BRICKS",
        [(0x7A, 0x4E, 0x16), (0x96, 0x62, 0x1E), (0x96, 0x62, 0x1E), (0xB4, 0x7A, 0x28)],
        (0x4C, 0x2E, 0x0C), [(0xE8, 0xA8, 0x3C), (0xFF, 0xC8, 0x5C)], 0.04, 0.14),
]

assert len(MATERIALS) == 14
assert len({m.mid for m in MATERIALS}) == 14
# Task spec: distinct map colors per material (verified against the Yarn 1.21.9 MapColor
# constants via javap).
...
def hb_material(mat: Mat) -> None:
    """One overview entry per MATERIAL covering its whole 16-block set."""
    HANDBOOK.append((
        "blocks", f"pyrestone/set_{mat.mid}", f"{NS}:{mat.mid}", None, None, None, 0,
        f"The {mat.en} set for Inferno builds: rough, polished and brick families "
        "(block, stairs, slab, wall) plus tiles, cracked and chiseled bricks and a pillar.",
        f"Das {mat.de}-Set f\u00fcr Inferno-Bauten: raue, polierte und Ziegel-Familien "
        "(Block, Treppe, Stufe, Mauer) sowie Fliesen, rissige und gemei\u00dfelte Ziegel "
        "und eine S\u00e4ule."))


def hb_shaped(name: str, grid: list, result: str, count: int) -> None:
    HANDBOOK.append(("blocks", f"pyrestone/{name}", f"{NS}:{result}", f"pyrestone/{name}",
...
def emit_recipes() -> int:
    ...
    for i, mat in enumerate(MATERIALS):
        ...
        # material[0] (pyrestone) is the chain ENTRY: crafted from already-obtainable
        # Inferno materials (cinderstone terrain corners + ember_dust centre) so the
        # 14-link chain has an entry point and no crafting cycle. Materials i >= 1 use
        # 4x vanilla flavor stone around the PREVIOUS material's base block.
        if i == 0:
            corner, centre = f"{ci}cinderstone", f"{ci}ember_dust"
        else:
...
# ---------------------------------------------------------------------------
# Textures (16x16, deterministic; distinct per-material palettes; genlib helpers).
# ---------------------------------------------------------------------------

# Flame glyph stamped on chiseled bricks (accent outline + hot core).
FLAME_GLYPH = [(8, 3), (7, 4), (8, 4), (6, 5), (7, 5), (9, 5), (5, 6), (6, 6), (8, 6),
               (9, 6), (10, 6), (5, 7), (6, 7), (7, 7), (9, 7), (10, 7), (4, 8), (5, 8),
               (7, 8), (8, 8), (9, 8), (10, 8), (11, 8), (5, 9), (6, 9), (7, 9), (8, 9),
               (9, 9), (10, 9), (6, 10), (7, 10), (8, 10), (9, 10), (7, 11), (8, 11)]
FLAME_CORE = [(7, 8), (8, 8), (7, 9), (8, 9), (8, 10)]
...
def emit_java() -> None:
    ...
    feature_src = genlib.java_feature_class(
        "pyrestone", "PyrestoneFeature", feature_doc,
        families=families,
        blocks=blocks,
        ...
        handbook_class="PyrestoneHandbook",
    )
    (FEATURE_DIR / "PyrestoneFeature.java").write_text(feature_src, encoding="utf-8")
    ...
    handbook_src = genlib.java_handbook_class("pyrestone", "PyrestoneHandbook", handbook_doc,
                                              HANDBOOK)
    (FEATURE_DIR / "PyrestoneHandbook.java").write_text(handbook_src, encoding="utf-8")


# ---------------------------------------------------------------------------
# Hook file (devtools/hooks/pyrestone.txt; format per devtools/hooks/README.md).
# ---------------------------------------------------------------------------

def emit_hooks(ids: list, recipe_count: int) -> None:
    lines = ["# pyrestone feature hooks (format: devtools/hooks/README.md)", "",
             "[init]",
    ...
    lines += ["", "[recipe-dir]", "pyrestone", "",
              "[counts]",
              f"blocks: {len(ids)}",
              "items: 0",
              f"recipes: {recipe_count}",
              f"handbook-entries: {len(HANDBOOK)}", ""]
    path = ROOT / "devtools" / "hooks" / "pyrestone.txt"
...
def main() -> None:
    ...
    genlib.lang_fragments(ASSETS, "pyrestone", lang_en, lang_de)

    write_json(ROOT / "devtools" / "tagfrag" / "pyrestone.json", {
    ...
    # 14 material overviews + one entry per recipe JSON.
    assert len(HANDBOOK) == 14 + recipe_count, \
        f"expected {14 + recipe_count} handbook entries, got {len(HANDBOOK)}"
    print(f"pyrestone_gen: assets generated for {len(ids)} blocks "
