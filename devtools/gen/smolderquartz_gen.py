#!/usr/bin/env python3
"""Asset + code generator for the COPPER INFERNO "smolderquartz" feature (224 building blocks).

14 crystal / glassy-stone materials (smolder_quartz, cinder_crystal, flame_opal,
ember_prism, ashglass_stone, glowspar, heatlens_stone, magma_geode, pyrite_crystal,
sulfur_crystal, obsid_glassblock, ember_amber, fire_agate, lava_pearl); each ships a
16-block set: the base cube family, the polished family and the bricks family
(base/slab/stairs/wall each), plus tiles, cracked bricks, chiseled bricks and a pillar.
Every block is a plain opaque requires-tool stone block (any glow/translucency is texture
flavor only, to stay audit-simple).
...
noise is seeded per texture name via genlib.rng_for). Emits by DEFAULT (no flags):
  ...
  - 16x16 block textures (Pillow, deterministic crystalline facet noise, distinct
    palette per material)
  - loot tables (drop-self; slabs use the vanilla double-drops-2 format)
  - recipes (data/copper_inferno/recipe/smolderquartz/*.json, 30 per material = 420)
  - lang fragments: assets/copper_inferno/lang/fragments/smolderquartz.json (EN)
    and assets/copper_inferno/lang/fragments_de/smolderquartz.json (real German)
  - devtools/tagfrag/smolderquartz.json (mineable/pickaxe for all 224, walls for the 42)
  - src/main/java/.../feature/smolderquartz/SmolderQuartzFeature.java +
    SmolderQuartzHandbook.java (genlib.java_feature_class / genlib.java_handbook_class;
    literal ids only)
  - devtools/hooks/smolderquartz.txt (integration hook file)
...
RECIPES = DATA / "recipe" / "smolderquartz"
FEATURE_DIR = ROOT / "src" / "main" / "java" / "net" / "sonic0810" / "copperinferno" / "feature" / "smolderquartz"

# ---------------------------------------------------------------------------
# Materials. Each gets a DISTINCT deterministic crystalline palette:
...
MATERIALS = [
    Mat("smolder_quartz", "Smolder Quartz", "Schwelquarz", "Schwelquarz", "Polierter",
        "minecraft:quartz", "OFF_WHITE", "AMETHYST_BLOCK",
        [(0xB4, 0xA4, 0x96), (0xCC, 0xBC, 0xAC), (0xCC, 0xBC, 0xAC), (0xE4, 0xD6, 0xC6)],
        (0x8A, 0x7C, 0x70), [(0xFF, 0x7A, 0x2F), (0xFF, 0xE2, 0xC8)], 0.03, 0.08),
    Mat("cinder_crystal", "Cinder Crystal", "Zunderkristall", "Zunderkristall", "Polierter",
        "minecraft:charcoal", "DULL_RED", "AMETHYST_BLOCK",
        [(0x33, 0x26, 0x2A), (0x44, 0x33, 0x38), (0x44, 0x33, 0x38), (0x58, 0x43, 0x49)],
        (0x1E, 0x14, 0x18), [(0xE2, 0x58, 0x22), (0xFF, 0x8A, 0x5A)], 0.04, 0.12),
    Mat("flame_opal", "Flame Opal", "Flammenopal", "Flammenopal", "Polierter",
        "minecraft:fire_charge", "ORANGE", "CALCITE",
        [(0x9E, 0x4A, 0x2A), (0xBC, 0x5E, 0x34), (0xBC, 0x5E, 0x34), (0xD8, 0x77, 0x42)],
        (0x6E, 0x30, 0x1C), [(0x64, 0xC8, 0xB4), (0xFF, 0xB1, 0x6B)], 0.05, 0.14),
    Mat("ember_prism", "Ember Prism", "Glutprisma", "Glutprismen", "Poliertes",
        "minecraft:amethyst_shard", "BRIGHT_RED", "AMETHYST_BLOCK",
        [(0x6A, 0x2E, 0x52), (0x84, 0x3C, 0x66), (0x84, 0x3C, 0x66), (0xA0, 0x4E, 0x7C)],
        (0x44, 0x1C, 0x36), [(0xFF, 0x7A, 0x2F), (0xE8, 0x9A, 0xD0)], 0.04, 0.10),
    Mat("ashglass_stone", "Ashglass Stone", "Aschglasstein", "Aschglasstein", "Polierter",
        "minecraft:glass", "LIGHT_GRAY", "CALCITE",
        [(0x7E, 0x82, 0x84), (0x96, 0x9A, 0x9C), (0x96, 0x9A, 0x9C), (0xB2, 0xB6, 0xB8)],
        (0x5A, 0x5E, 0x60), [(0xD8, 0xDE, 0xE0), (0xE2, 0x58, 0x22)], 0.03, 0.05),
    Mat("glowspar", "Glowspar", "Leuchtspat", "Leuchtspat", "Polierter",
        "minecraft:glowstone_dust", "PALE_YELLOW", "CALCITE",
        [(0xA8, 0xA4, 0x5E), (0xC2, 0xBE, 0x72), (0xC2, 0xBE, 0x72), (0xDC, 0xD8, 0x8A)],
        (0x7A, 0x76, 0x40), [(0xF4, 0xF0, 0xA8), (0xFF, 0xFC, 0xD0)], 0.05, 0.16),
    Mat("heatlens_stone", "Heatlens Stone", "Hitzelinsenstein", "Hitzelinsenstein", "Polierter",
        "minecraft:gold_nugget", "GOLD", "STONE",
        [(0x8C, 0x62, 0x24), (0xA8, 0x78, 0x2E), (0xA8, 0x78, 0x2E), (0xC4, 0x90, 0x3C)],
        (0x5E, 0x40, 0x16), [(0xFF, 0xD2, 0x6E), (0xFF, 0x7A, 0x2F)], 0.04, 0.12),
    Mat("magma_geode", "Magma Geode", "Magmageode", "Magmageoden", "Polierte",
        "minecraft:magma_cream", "DARK_RED", "NETHER_BRICKS",
        [(0x46, 0x20, 0x18), (0x5A, 0x2A, 0x1E), (0x5A, 0x2A, 0x1E), (0x70, 0x36, 0x26)],
        (0x2A, 0x12, 0x0E), [(0xFF, 0x64, 0x28), (0xFF, 0x9A, 0x40)], 0.06, 0.22),
    Mat("pyrite_crystal", "Pyrite Crystal", "Pyritkristall", "Pyritkristall", "Polierter",
        "minecraft:raw_gold", "TERRACOTTA_YELLOW", "STONE",
        [(0x7E, 0x6A, 0x30), (0x98, 0x80, 0x3C), (0x98, 0x80, 0x3C), (0xB4, 0x98, 0x4A)],
        (0x54, 0x46, 0x20), [(0xE8, 0xD0, 0x70), (0xFF, 0xEC, 0x9A)], 0.05, 0.10),
    Mat("sulfur_crystal", "Sulfur Crystal", "Schwefelkristall", "Schwefelkristall", "Polierter",
        "minecraft:gunpowder", "YELLOW", "CALCITE",
        [(0xA8, 0x96, 0x2E), (0xC4, 0xB0, 0x3A), (0xC4, 0xB0, 0x3A), (0xDE, 0xCA, 0x4C)],
        (0x76, 0x68, 0x1E), [(0xF2, 0xE6, 0x74), (0xE2, 0x58, 0x22)], 0.04, 0.10),
    Mat("obsid_glassblock", "Obsid Glassblock", "Obsidglasblock", "Obsidglas", "Polierter",
        "minecraft:obsidian", "BLACK", "DEEPSLATE",
        [(0x16, 0x10, 0x1E), (0x22, 0x1A, 0x2E), (0x22, 0x1A, 0x2E), (0x30, 0x26, 0x40)],
        (0x0C, 0x08, 0x12), [(0x6E, 0x4E, 0x9E), (0x9A, 0x78, 0xC8)], 0.03, 0.06),
    Mat("ember_amber", "Ember Amber", "Glutbernstein", "Glutbernstein", "Polierter",
        "minecraft:resin_clump", "TERRACOTTA_ORANGE", "CALCITE",
        [(0x9A, 0x5E, 0x1E), (0xB6, 0x74, 0x26), (0xB6, 0x74, 0x26), (0xD2, 0x8C, 0x32)],
        (0x6A, 0x3E, 0x14), [(0xFF, 0xB1, 0x6B), (0xFF, 0xD9, 0x9E)], 0.04, 0.12),
    Mat("fire_agate", "Fire Agate", "Feuerachat", "Feuerachat", "Polierter",
        "minecraft:blaze_powder", "TERRACOTTA_RED", "STONE",
        [(0x7C, 0x30, 0x22), (0x96, 0x3E, 0x2A), (0x96, 0x3E, 0x2A), (0xB2, 0x4E, 0x34)],
        (0x50, 0x1E, 0x16), [(0xE2, 0x58, 0x22), (0xF0, 0xA8, 0x74)], 0.04, 0.12),
    Mat("lava_pearl", "Lava Pearl", "Lavaperle", "Lavaperlen", "Polierte",
        "minecraft:ender_pearl", "DARK_CRIMSON", "AMETHYST_BLOCK",
        [(0x66, 0x1A, 0x28), (0x80, 0x24, 0x34), (0x80, 0x24, 0x34), (0x9C, 0x30, 0x44)],
        (0x40, 0x0E, 0x1A), [(0xFF, 0x8A, 0x64), (0xF2, 0xD8, 0xCC)], 0.05, 0.14),
]
...
def hb_family(base: str) -> None:
    ...
    HANDBOOK.append(("blocks", f"smolderquartz/family_{base}", f"{NS}:{base}", None, None, None, 0,
...
def hb_shaped(name: str, grid: list, result: str, count: int) -> None:
    HANDBOOK.append(("blocks", f"smolderquartz/{name}", f"{NS}:{result}", f"smolderquartz/{name}",
...
def emit_recipes() -> int:
    ...
    for i, mat in enumerate(MATERIALS):
        ...
        # Stonecutting from the base rough crystal for every family member.
...
# ---------------------------------------------------------------------------
# Textures (16x16, deterministic; distinct per-material crystalline palettes;
# genlib helpers plus a facet-streak overlay for the raw crystal look).
# ---------------------------------------------------------------------------

# Crystal shard glyph stamped on chiseled bricks (accent outline + bright core):
# one big shard pointing up plus a small side shard.
SHARD_GLYPH = [(7, 3), (6, 4), (8, 4), (6, 5), (8, 5), (5, 6), (9, 6), (5, 7), (9, 7),
               (5, 8), (9, 8), (6, 9), (8, 9), (6, 10), (7, 10), (8, 10), (7, 11),
               (11, 6), (10, 7), (12, 7), (10, 8), (12, 8), (11, 9), (11, 10)]
SHARD_CORE = [(7, 5), (7, 6), (7, 7), (7, 8), (11, 7), (11, 8)]


def facet_streaks(img, rng, light, n=3):
    """Short diagonal facet lines (crystal planes), deterministic via the seeded rng."""
    for _ in range(n):
        x, y = rng.randrange(16), rng.randrange(16)
        step = rng.choice([-1, 1])
        for i in range(rng.randrange(4, 8)):
            img.putpixel(((x + i) % 16, (y + step * i) % 16), light)


def tex_base(mat: Mat, rng):
    """Rough crystal: mottled base, diagonal facet streaks, sparse glint accents."""
    img = genlib.new_canvas(rng, mat.shades)
    facet_streaks(img, rng, mat.shades[-1])
...
def tex_tiles(mat: Mat, rng):
    """2x2 grid of 8x8 tiles: dark grout, accent glints at the crossings."""
    img = genlib.new_canvas(rng, mat.shades)
    grout = []
    for y in range(16):
        for x in range(16):
...
def tex_chiseled(mat: Mat, rng):
    ...
    for x, y in SHARD_GLYPH:
        img.putpixel((x, y), mat.accents[0])
    for x, y in SHARD_CORE:
...
def emit_java() -> None:
    ...
    feature_src = genlib.java_feature_class(
        "smolderquartz", "SmolderQuartzFeature", feature_doc,
        families=families,
        blocks=blocks,
        ...
        handbook_class="SmolderQuartzHandbook",
    )
    (FEATURE_DIR / "SmolderQuartzFeature.java").write_text(feature_src, encoding="utf-8")
    ...
    handbook_src = genlib.java_handbook_class("smolderquartz", "SmolderQuartzHandbook",
                                              handbook_doc, HANDBOOK)
    (FEATURE_DIR / "SmolderQuartzHandbook.java").write_text(handbook_src, encoding="utf-8")


# ---------------------------------------------------------------------------
# Hook file (devtools/hooks/smolderquartz.txt; format per devtools/hooks/README.md).
# ---------------------------------------------------------------------------

def emit_hooks(ids: list, recipe_count: int) -> None:
    lines = ["# smolderquartz feature hooks (format: devtools/hooks/README.md)", "",
             "[init]",
    ...
    lines += ["", "[recipe-dir]", "smolderquartz", "",
              "[counts]",
              f"blocks: {len(ids)}",
              "items: 0",
              f"recipes: {recipe_count}",
              f"handbook-entries: {len(HANDBOOK)}", ""]
    path = ROOT / "devtools" / "hooks" / "smolderquartz.txt"
...
def main() -> None:
    ...
    genlib.lang_fragments(ASSETS, "smolderquartz", lang_en, lang_de)

    write_json(ROOT / "devtools" / "tagfrag" / "smolderquartz.json", {
    ...
    print(f"smolderquartz_gen: assets generated for {len(ids)} blocks "
