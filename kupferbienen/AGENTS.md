...
- **Bee/apiary radius mechanics** (`KupferstockBlockEntity`): every 20 ticks the Kupferstock counts living Kupfer-/Gruenspanbienen within **4 blocks** and requires >=1 Kupferbluete within **4 blocks**; progress += `20 * min(bees, 5)` toward a 600-unit product (so 2 bees ≈ 15 s per item; hold when the 3-slot inventory is full; 20% Gruenspanpollen chance once a Gruenspanbiene contributed). Summoned bees wander — in tests, re-`/tp` them next to the apiary every ~30 s or summon them with `{NoAI:1b}`. A bee only counts for its **nearest** Kupferstock (ties broken by `BlockPos.asLong()`), so stacking multiple apiaries around one swarm does NOT multiply output — in tests expect the second-nearest apiary to stay empty.
- **Kupferbluete is a `PlantBlock`**: it can only sit on `#minecraft:dirt` or farmland and pops with drops when its support is removed. Any test that `/setblock`s a kupferbluete onto the obsidian platform must place `minecraft:dirt` underneath first, or the flower breaks instantly.
...
- Startup marker to grep for: `[KUPFERBIENEN] Registered oxidation brewing chain (5 item recipes)`. Recipe check: the server should log `Loaded 1468 recipes` (vanilla 1461 + this mod's 7 crafting recipes) and `Loaded 1584 advancements` (vanilla 1574 + this mod's 6 recipe-unlock + 4 chain advancements).
- Waxed-copper crafting deliberately yields `waxed_cut_copper` (9 Kupferwaben -> **4 waxed_cut_copper**), NOT waxed_copper_block: a waxed copper block would un-craft into 9 copper ingots, letting the comb->block->ingot loop beat the intended 3-combs-per-ingot rate (arbitrage). Cut copper has no path back to ingots.
...
## Mega-expansion layout

- **Creative tabs** (`core/ModCreativeTab.java`, lazy `iconFor` tolerates not-yet-registered icon ids): `BIENEN_KEY` (`kupferbienen:bienen`, icon kupferwabe — bees/flora content), `ALCHEMIE_KEY` (`kupferbienen:alchemie`, icon trank_der_oxidation — brewing content), `MASCHINEN_KEY` (`kupferbienen:maschinen`, icon wabenpresse — machinery content). There is no MAIN tab anymore.
- **Init order** (`Kupferbienen.onInitialize()`): `ModCreativeTab` -> `FloraFeature` -> `BeesFeature` -> `MachinesFeature` -> `PotionsFeature`. Tabs first (features hook `ItemGroupEvents` on their keys); flora before bees (bee content may reference flora flowers/crops); machines after bees (machines process bee produce); potions last (brewing chain references bee items and possibly machine outputs). Client (`KupferbienenClient`): `BeesFeatureClient` -> `FloraFeatureClient` -> `PotionsFeatureClient`.
- **Worldgen features** register through `core/ModFeatures.register(path, feature)` into `Registries.FEATURE` (vanilla `Feature.register` is private).
- **Generator tables** (`devtools/gen/kupferbienen_gen.py`) — later workers APPEND rows, they do not write new painter code: `BEE_PALETTES` (entity remaps), `SIMPLE_FLOWERS` (-> `flower()` texture + `emit_flower_block` JSON), `MACHINE_BLOCKS` (-> `machine_face()` side/top textures + `emit_machine_block` JSON), `CROPS` (-> `crop_stage()` stage textures + `emit_crop` JSON). Textures emit always; JSON only with `--write-json`. Determinism contract: `rng_for(name)` per texture, rng consumed exactly once per canvas pixel; motif/glyph overlays must NOT consume rng.
- **Lang parity one-liner** (must print OK): `python3 -c "import json;a=json.load(open('src/main/resources/assets/kupferbienen/lang/en_us.json'));b=json.load(open('src/main/resources/assets/kupferbienen/lang/de_de.json'));d=set(a)^set(b);print('OK' if not d else d)"`
