...
public final class ModBlocks {
	...
	/**
	 * Registers a block under {@code copper_inferno:<path>}. The registry key is applied to the
	 ...
	 * using the block-prefixed translation key ({@code block.copper_inferno.<path>}).
	 */
	public static Block register(String path, Function<AbstractBlock.Settings, Block> factory, AbstractBlock.Settings settings, boolean withBlockItem) {
		RegistryKey<Block> key = RegistryKey.of(RegistryKeys.BLOCK, CopperInferno.id(path));
		Block block = factory.apply(settings.registryKey(key));
		Registry.register(Registries.BLOCK, key, block);
		if (withBlockItem) {
			RegistryKey<Item> itemKey = RegistryKey.of(RegistryKeys.ITEM, CopperInferno.id(path));
