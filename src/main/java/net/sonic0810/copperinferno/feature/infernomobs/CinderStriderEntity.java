...
/**
 * A cinder-crusted strider that wades the Inferno's slag seas. Behavior is vanilla strider
 * (lava walking, saddling, warped-fungus breeding are all inherited) except that breeding
 * produces cinder striders (not vanilla ones) and natural spawns never roll the vanilla
 * zombified-piglin/baby-strider jockeys. Drops Strider Shells
...
public class CinderStriderEntity extends StriderEntity {
	 ...
	 * {@code EntityType.STRIDER} (bytecode-verified), so bred cinder striders would produce
	 * vanilla striders. Covariant override returning our own type instead.
	 */
	@Override
	public CinderStriderEntity createChild(ServerWorld world, PassiveEntity entity) {
		return new CinderStriderEntity(InfernoMobsFeature.CINDER_STRIDER, world);
	}
	 ...
	 * baby strider as a jockey — both would attach vanilla mobs to a cinder strider. Only the
