...
/**
 * Basalt Strider - a heavyset strider armored in basalt plates, plodding through the deepest lava pools. Behavior is vanilla strider (lava walking, saddling, warped-fungus breeding are all
 * inherited) except that breeding produces basalt strider babies (not vanilla striders) and natural
 * spawns never roll the vanilla zombified-piglin/baby-strider jockeys (same proven pattern as
 * the cinder strider). Drops Basalt Scale ({@code loot_table/entities/basalt_strider.json}).
 */
public class BasaltStriderEntity extends StriderEntity {
	public BasaltStriderEntity(EntityType<? extends StriderEntity> type, World world) {
		super(type, world);
	}

	/**
	 * Vanilla {@code StriderEntity.createChild(ServerWorld, PassiveEntity)} hard-codes
	 * {@code EntityType.STRIDER} (bytecode-verified), so bred basalt strider pairs would produce
	 * vanilla striders. Covariant override returning our own type instead.
	 */
	@Override
	public BasaltStriderEntity createChild(ServerWorld world, PassiveEntity entity) {
		return new BasaltStriderEntity(MoltenFaunaFeature.BASALT_STRIDER, world);
	}
	 ...
	 * baby strider as a jockey - both would attach vanilla mobs to a basalt strider. Only the
