...
/**
 * Magma Strider - a magma-crusted strider wading the lava of the Slag Sea and its neighbors. Can be saddled and ridden. Behavior is vanilla strider (lava walking, saddling, warped-fungus breeding are all
 * inherited) except that breeding produces magma strider babies (not vanilla striders) and natural
 * spawns never roll the vanilla zombified-piglin/baby-strider jockeys (same proven pattern as
 * the cinder strider). Drops Magma Carapace ({@code loot_table/entities/magma_strider.json}).
 */
public class MagmaStriderEntity extends StriderEntity {
	public MagmaStriderEntity(EntityType<? extends StriderEntity> type, World world) {
		super(type, world);
	}

	/**
	 * Vanilla {@code StriderEntity.createChild(ServerWorld, PassiveEntity)} hard-codes
	 * {@code EntityType.STRIDER} (bytecode-verified), so bred magma strider pairs would produce
	 * vanilla striders. Covariant override returning our own type instead.
	 */
	@Override
	public MagmaStriderEntity createChild(ServerWorld world, PassiveEntity entity) {
		return new MagmaStriderEntity(MoltenFaunaFeature.MAGMA_STRIDER, world);
	}
	 ...
	 * baby strider as a jockey - both would attach vanilla mobs to a magma strider. Only the
