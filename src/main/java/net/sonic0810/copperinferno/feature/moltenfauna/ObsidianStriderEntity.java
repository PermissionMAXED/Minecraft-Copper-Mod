...
/**
 * Obsidian Strider - a rare strider sheathed in glassy obsidian, the toughest of the lava waders. Behavior is vanilla strider (lava walking, saddling, warped-fungus breeding are all
 * inherited) except that breeding produces obsidian strider babies (not vanilla striders) and natural
 * spawns never roll the vanilla zombified-piglin/baby-strider jockeys (same proven pattern as
 * the cinder strider). Drops Obsidian Plating ({@code loot_table/entities/obsidian_strider.json}).
 */
public class ObsidianStriderEntity extends StriderEntity {
	public ObsidianStriderEntity(EntityType<? extends StriderEntity> type, World world) {
		super(type, world);
	}

	/**
	 * Vanilla {@code StriderEntity.createChild(ServerWorld, PassiveEntity)} hard-codes
	 * {@code EntityType.STRIDER} (bytecode-verified), so bred obsidian strider pairs would produce
	 * vanilla striders. Covariant override returning our own type instead.
	 */
	@Override
	public ObsidianStriderEntity createChild(ServerWorld world, PassiveEntity entity) {
		return new ObsidianStriderEntity(MoltenFaunaFeature.OBSIDIAN_STRIDER, world);
	}
	 ...
	 * baby strider as a jockey - both would attach vanilla mobs to a obsidian strider. Only the
