...
/**
 * Slag Strider - a strider caked in cooling slag, at home wherever the Slag Sea churns. Behavior is vanilla strider (lava walking, saddling, warped-fungus breeding are all
 * inherited) except that breeding produces slag strider babies (not vanilla striders) and natural
 * spawns never roll the vanilla zombified-piglin/baby-strider jockeys (same proven pattern as
 * the cinder strider). Drops Slag Husk ({@code loot_table/entities/slag_strider.json}).
 */
public class SlagStriderEntity extends StriderEntity {
	public SlagStriderEntity(EntityType<? extends StriderEntity> type, World world) {
		super(type, world);
	}

	/**
	 * Vanilla {@code StriderEntity.createChild(ServerWorld, PassiveEntity)} hard-codes
	 * {@code EntityType.STRIDER} (bytecode-verified), so bred slag strider pairs would produce
	 * vanilla striders. Covariant override returning our own type instead.
	 */
	@Override
	public SlagStriderEntity createChild(ServerWorld world, PassiveEntity entity) {
		return new SlagStriderEntity(MoltenFaunaFeature.SLAG_STRIDER, world);
	}
	 ...
	 * baby strider as a jockey - both would attach vanilla mobs to a slag strider. Only the
